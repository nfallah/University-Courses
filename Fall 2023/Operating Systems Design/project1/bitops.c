...
static unsigned int get_top_bits(unsigned int value,  int num_bits)
{
	...
	int num_shifts = sizeof(unsigned int) * 8 - num_bits; // How many right shifts should occur to obtain top n bits
...
static void set_bit_at_index(char *bitmap, int index)
{
	...
	int set_index = index % 8;
	char bit_mask = 1 << set_index;
	bitmap[arr_index] = bitmap[arr_index] | bit_mask;
	return;
}
...
static int get_bit_at_index(char *bitmap, int index)
{
	...
	int set_index = index % 8;
	char bit_mask = 1 << set_index;
	return (bitmap[arr_index] & bit_mask) != 0;
}
